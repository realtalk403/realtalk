...
public class RoomLoader extends AsyncTask<String, String, ChatRoomResultSet> {
	...
	protected ChatRoomResultSet doInBackground(String... params) {			
		...
		this.selectRoomActivity2.setRgchatroominfo(crrsNear.rgcriGet());

		selectroomactivity.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				RoomLoader.this.selectRoomActivity2.getUnJoinedAdapter().clear();
				RoomLoader.this.selectRoomActivity2.getJoinedAdapter().clear();

				for (int i = 0; i < RoomLoader.this.selectRoomActivity2.getRgchatroominfo().size(); i++) {
					if (!ChatController.getInstance().fIsAlreadyJoined(RoomLoader.this.selectRoomActivity2.getRgchatroominfo().get(i))) {
						RoomLoader.this.selectRoomActivity2.getUnJoinedAdapter().add(RoomLoader.this.selectRoomActivity2.getRgchatroominfo().get(i));
					}
				}
				
				List<ChatRoomInfo> rgJoinedRooms = ChatController.getInstance().getChatRooms();
				for (int i = 0; i < rgJoinedRooms.size(); i++) {
					RoomLoader.this.selectRoomActivity2.getJoinedAdapter().add(rgJoinedRooms.get(i));
				}
			}
		});
